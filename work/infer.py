        ...
        for data in loader():
            ...
            for i in range(args.c_dim):
                ...
                for j in range(len(label_trg_tmp)):
                    label_trg_tmp[j][i] = 1.0 - label_trg_tmp[j][i]     # 把测试集标签取反:比如到头发时，取反头发的特征
                ...
                label_org_tmp = list(
                    map(lambda x: ((x * 2) - 1) * 0.5, np.array(label_org)))  # 这里的0.5为变化程度，取值0-1，超过1会变形，个人觉得可以取到1.1，看上去变形不是很严重
                label_trg_tmp = list(
                    map(lambda x: ((x * 2) - 1) * 1.1, label_trg_tmp))
